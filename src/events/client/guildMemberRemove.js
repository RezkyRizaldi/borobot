...
module.exports = {
	...
	async execute(member) {
		...
		await LeaveLogger.send({ embeds: [leaveMessage] }).catch((err) => console.error(err));
		...
		if (kickLog.target.id === user.id) return KickLogger.send({ embeds: [kickMessage] }).catch((err) => console.error(err));
	},
};
