...
module.exports = {
	...
	async execute(ban) {
		...
		const botColor = await ban.guild.members.fetch(ban.client.user.id).then((res) => res.displayHexColor);

		const message = new EmbedBuilder()
			.setDescription(`${ban.user.tag} has been unbanned by ${UnbanLog.executor}`)
		...
		if (UnbanLog.target.id === ban.user.id) return UnbanLogger.send({ embeds: [message] }).catch((err) => console.error(err));
	},
};
