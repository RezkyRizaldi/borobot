const { AuditLogEvent, bold, channelMention, EmbedBuilder, Events, time, TimestampStyles, userMention, WebhookClient } = require('discord.js');
const pluralize = require('pluralize');

const { applyMessageType, groupMessageByAuthor, groupMessageByType } = require('../../utils');
...
module.exports = {
	...
	async execute(messages) {
		...
		if (!messages.every((message) => message.author.id === messages.first().author.id)) {
			const groupedMessages = groupMessageByType(groupMessageByAuthor(messages));

			const response = `${groupedMessages
				.flatMap(
					(arrMessage) =>
						`${arrMessage.reduce((acc, curr) => acc + curr.length, 0)} ${pluralize(
							'message',
							arrMessage.reduce((acc, curr) => acc + curr.length, 0),
						)} from ${userMention(arrMessage[0][0].author.id)} was ${bold('deleted')} by ${bulkDeleteLog.executor} in ${channelMention(arrMessage[0][0].channel.id)} at ${time(Math.floor(Date.now() / 1000), TimestampStyles.RelativeTime)}`,
				)
				.join('\n')}\n${bold('Deleted Messages')}\n${groupedMessages
				.flatMap((arrMessage) => `from ${userMention(arrMessage[0][0].author.id)}\n${arrMessage.map((msgs) => msgs.map((message) => applyMessageType(message)).join('\n')).join('\n')}`)
				.join('\n\n')}`;
		...
		const response = `${messages.size} messages from ${messages.first().author} was ${bold('deleted')} by ${bulkDeleteLog.executor} in ${messages.first().channel} at ${time(
			Math.floor(Date.now() / 1000),
			TimestampStyles.RelativeTime,
		)}\n${bold('Deleted Messages')}\n${messages.map((message) => applyMessageType(message)).join('\n')}`;
