...
module.exports = {
	data: new SlashCommandBuilder()
		...
		.addChannelOption((option) => option.setName('channel').setDescription('🔊 The voice channel destination.').setRequired(true).addChannelTypes(ChannelType.GuildVoice))
		.addStringOption((option) => option.setName('reason').setDescription('📃 The reason for moving the member.')),
	type: 'Chat Input',
	...
	async execute(interaction) {
		...
		if (member.voice.channelId === channel.id) return interaction.reply({ content: `This member is already in ${channel}.`, ephemeral: true });
