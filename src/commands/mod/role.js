	...
	data: new SlashCommandBuilder()
		...
		.addSubcommand((subcommand) =>
			subcommand
				...
				.addRoleOption((option) => option.setName('role').setDescription('‍🛠️ The role to add.').setRequired(true))
		...
		.addSubcommand((subcommand) =>
			subcommand
				...
				.addUserOption((option) => option.setName('member').setDescription('👤 The member whose role to be removed.').setRequired(true))
				.addRoleOption((option) => option.setName('role').setDescription('🛠️ The role to remove.').setRequired(true))
	...
	async execute(interaction) {
		...
		switch (options.getSubcommand()) {
			case 'add':
				...
				if (member.roles.cache.has(role.id)) return interaction.reply({ content: `This member already have ${role} role.`, ephemeral: true });
			...
			case 'remove':
				...
				if (!member.roles.cache.has(role.id)) return interaction.reply({ content: `This member doesn't have ${role} role.`, ephemeral: true });
